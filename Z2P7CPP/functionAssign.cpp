#include<iostream>
using namespace std;
string itemList[5][3];
int count ;
void initializeItemData(){  
  cout<<"Enter number of data:";
  cin>>count;
    for (int i=0;i<count;i++){ 
        for (int j=0;j<3;j++){ 
             if(j==0){
             cout<<"Enter barcode: ";
             cin>>itemList[i][j];
            }else if(j==1){
             cout<<"Enter name: ";
             cin>>itemList[i][j];
            }else{
             cout<<"Enter price: ";
              cin>>itemList[i][j];
          }
       }
    }
  }
    void displayItemData(){
      cout<<"barcode"<<'\t'<<"Name"<<'\t'<<"Price"<<endl;
         for(int i=0;i<count;i++){
           for (int j=0;j<3;j++){ 
             cout<<itemList[i][j]<<"\t";
          }
        cout<<endl;
      } 
    }
    void searchByBarcode(){
      string Barcode;
      cout<<"Enter Barcode you want to search:";
      cin>>Barcode;
      // cout<<"Barcode"<<"\tName"<<"\tPrice"<<endl;
      // if(itemList[0][0]==Barcode){
      //   cout<<itemList[0][0]<<"\t"<<itemList[0][1]<<"\t"<<itemList[0][2]<<endl;
      // }if(itemList[1][0]==Barcode){
      //   cout<<itemList[1][0]<<"\t"<<itemList[1][1]<<"\t"<<itemList[1][2]<<endl;
      // }if(itemList[2][0]==Barcode){
      //   cout<<itemList[2][0]<<"\t"<<itemList[2][1]<<"\t"<<itemList[2][2]<<endl;
      // }if(itemList[3][0]==Barcode){
      //   cout<<itemList[3][0]<<"\t"<<itemList[3][1]<<"\t"<<itemList[3][2]<<endl;
      // }if(itemList[4][0]==Barcode){
      //   cout<<itemList[4][0]<<"\t"<<itemList[4][1]<<"\t"<<itemList[4][2]<<endl;
      // }  
      for(int i=0;i<count;i++){
      if(Barcode==itemList[i][0]){
         cout<<"barcode"<<'\t'<<"Name"<<'\t'<<"Price"<<endl;
         cout<<itemList[i][0]<<"\t"<<itemList[i][1]<<"\t"<<itemList[i][2];
      }else{
         cout<<endl;
        }   
      }
    }
      int main(){
      initializeItemData();
      displayItemData();
      searchByBarcode();
    return 0;
  }